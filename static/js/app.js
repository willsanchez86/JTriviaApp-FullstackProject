...
async function playGame() {
  ...
  document.querySelector('.end_round').addEventListener('click', function () {
    if (
      confirm(
        'Are you sure you want to end current game? All progress will be lost!'
      )
    ) {
    ...
    if (e.target.className === 'qbutton') {
      ...
      let timer = setInterval(() => {
        ...
        if (countSeconds <= 0) {
          ...
          setMessage(
            message,
            `Out of Time! The correct answer is ${modalAnswer.textContent}`,
            'red'
          );
      ...
      let newCheckAnswer = function () {
        checkAnswer(e, currentSquare);
      };
...
function storeGameInLocalStorage(gameData) {
  ...
  localStorage.setItem(
    'filter_answers',
    JSON.stringify(gameData['filter_answers'])
  );
...
function checkAnswer(e) {
  ...
  if (answerInput.value === '') {
    ...
    if (answerInput.value.toUpperCase() === modalAnswer.textContent) {
      setMessage(
        message,
        `${answerInput.value.toUpperCase()} is Correct!`,
        'green'
      );
      updateScore(add);
    } else {
      ...
      setMessage(
        message,
        `WRONG! The correct answer is ${modalAnswer.textContent}`,
        'red'
      );
