...
// In order to ensure that all input fields (for both login and register pages) are valid at the same time and haven't reverted back, we needed to iterate over the nodelist to output an array of bools --> then re-enable the button only when all of the bools were true
